...
public class Voo {
    ...
    public void ReadFlight (File flight) {
        ...
        assert input != null;
     Aviao = setupFlight(input.nextLine());
    ...
    public void cancelReservation(int sequential_reservation_number) {
        ...
        char classeFlight;
        int j;
        boolean isExec = Aviao.getExecFlight() != null;

        j = 0;
        classeFlight = 'E';
        if (isExec) {
            for (int[] row : Aviao.getExecFlight()) {
                j++;
                for (int i = 0; i < row.length; i++) {
                    if (row[i] == sequential_reservation_number) {
                     Aviao.unmarkSeats(classeFlight, j, i);
                        execReserved--;
                    }
                    if (row[i] > sequential_reservation_number)
                     Aviao.updateSeatMark(classeFlight, j, i);
                }
            }
        }
        ...
        j = 0;
        classeFlight = 'T';
        for (int[] row : Aviao.getTouristFlight()) {
            j++;
            for (int i = 0; i < row.length; i++) {
                if (row[i] == sequential_reservation_number) {
                 Aviao.unmarkSeats(classeFlight, j, i);
    ...
    public void addReservation(String reserva) {
        ...
        if (classeFlight == 'E') {
            if (Aviao.getExecSize() - execReserved - reserved < 0)
        ...
        if (classeFlight == 'T') {
            if (Aviao.getTouristSize() - touristReserved - reserved < 0)
                PrintErrorReservation(classeFlight, reserved);
            else {
    ...
    private String[] fillSeats(char classeFlight, int reserved) {
        ...
        j = 0;
        for (int[] row : (classeFlight == 'E')? Aviao.getExecFlight() : Aviao.getTouristFlight()) {
            j++;
            if (Arrays.stream(row).sum() == 0)
                for (int i = 0; i < row.length; i++) {
                 Aviao.markSeats(classeFlight, j, i, nReserve);
                    reservedSeats[n-reserved] = ((char) (65 + i)) + "" + (j + ((!isExec || classeFlight == 'E')? 0 : Aviao.getExecFlight().length));
                    reserved--;
                    if (reserved == 0)
    ...
    public void PrintFlightMap() {
        boolean isExec = Aviao.getExecFlight() != null;
        int nFila = Aviao.getTouristNumFilas() + ((isExec)? Aviao.getExecNumFilas() : 0);
        int nSeat = Math.max(Aviao.getTouristNumLugares(), ((isExec)? Aviao.getExecNumLugares() : 0));
        ...
        for (int i = 0; i < nSeat; i++)
        {
            System.out.printf("\n%c", (char) (65 + i));
            if (isExec)
                for (int[] ints : Aviao.getExecFlight())
                    System.out.printf("%4s", ((i < Aviao.getExecNumLugares())) ? String.valueOf(ints[i]) : " ");
            for (int[] ints : Aviao.getTouristFlight())
                System.out.printf("%4s", ((i < Aviao.getTouristNumLugares())) ? String.valueOf(ints[i]) : " ");
        }
        System.out.println();
    }
    ...
    private void PrintFlightInformation() {
        ...
        if (Aviao.getExecSize() > 0)
            System.out.print(Aviao.getExecSize() + " lugares em classe Executiva; ");
        System.out.print(Aviao.getTouristSize() + " lugares em classe Turística.");
        if  (Aviao.getExecSize() <= 0)
    ...
    private void PrintErrorReservation(char classeFlight, int reserved) {
        ...
        if (classeFlight == 'E' && Aviao.getExecFlight() == null)
